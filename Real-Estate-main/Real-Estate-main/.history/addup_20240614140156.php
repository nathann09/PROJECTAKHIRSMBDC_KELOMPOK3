...
// Fungsi untuk mendapatkan upid selanjutnya
function getNextUpid($conn) {
    $sql = "SELECT MAX(upid) AS max_upid FROM upcoming_projects";
    $result = $conn->query($sql);
    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        return intval($row['max_upid']) + 1;
    } else {
        return 1; // Jika tidak ada proyek, mulai dari 1
    }
}
...
$sql = "INSERT INTO upcoming_projects (location, city, comp_time) 
        VALUES (?, ?, ?)";

$stmt = $conn->prepare($sql);

// Bind parameter ke prepared statement
$stmt->bind_param("sss", $loc, $city, $date);

// Set nilai parameter
$loc = "nilai lokasi"; // Ganti dengan nilai yang sesuai
$city = "nama kota"; // Ganti dengan nilai yang sesuai
$date = "tanggal selesai"; // Ganti dengan nilai yang sesuai

// Eksekusi prepared statement
if ($stmt->execute()) {
    // Redirect setelah berhasil memasukkan
    header('Location: upcomingprojects.php');
    exit();
} else {
    ...
    <form id="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <table cellpadding="7" width="50%" border="0" align="center" cellspacing="2" color="white">
            ...
            <tr>
                <td><b>Location:</b></td>
                <td>
    <input type="text" name="loc" size="30" value="<?php echo isset($loc) ? $loc : ''; ?>">
...
<tr>
    <td><b>City:</b></td>
    <td>
        <input type="text" name="city" size="30" value="<?php echo isset($city) ? $city : ''; ?>">
...
<tr>
    ...
    <td>
        <input type="text" name="date" placeholder="Months" size="30" value="<?php echo isset($date) ? $date : ''; ?>">
